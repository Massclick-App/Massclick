...
const S = ({
  ...
  sx,
  ...rest
}) => <Skeleton variant={variant} width={w} height={h} animation="wave" sx={{
  borderRadius: r ?? 2,
  flexShrink: 0,
...
const SkeletonCards = ({
  ...
  return <div className={cx("sk-hscroll")} style={{
    padding: "0 16px",
    height: c.containerH,
    display: 'flex',
    alignItems: 'center',
    contain: 'layout style paint'
            ...
            {[...Array(c.count)].map((_, i) => <div key={i} className={cx("service-card")} style={{
      width: c.cardW,
      height: c.cardH,
      flexShrink: 0
    }}>
  ...
  return <div style={{
    ...
    overflow: 'auto'
      ...
      overflow: "hidden"
                ...
                {[...Array(count)].map((_, i) => <div key={i} className={cx(isTrending ? "trending-search__card" : "popular-search__card")} style={{
        width: cardW,
        flexShrink: 0
      }}>
...
const LandingPage = React.memo(() => {
  ...
  const fallbackSeo = {
    ...
    description: "Find trusted local businesses, services, restaurants, hotels, and professionals near you on Massclick.",
  ...
  const [locationName, setLocationName] = useState(localStorage.getItem("selectedLocation") || "trichy");
  ...
  const drawerContent = <Box onClick={handleMobileMenuClose} sx={{
    textAlign: 'center'
  }}>
            <List>
                {['About Us', 'Services', 'Testimonials', 'Portfolio'].map(text => <ListItem button key={text}>
  ...
  return <>
      ...
      width: '100%'
                ...
                <CardsSearch isScrolled={isScrolled} locationName={locationName} setLocationName={setLocationName} searchTerm={searchTerm} setSearchTerm={setSearchTerm} categoryName={categoryName} setCategoryName={setCategoryName} />

                <main>
                <Box ref={heroSectionRef}>
                    <HeroSection locationName={locationName} setLocationName={setLocationName} searchTerm={searchTerm} setSearchTerm={setSearchTerm} categoryName={categoryName} setCategoryName={setCategoryName} setSearchResults={setSearchResults} />
                </Box>
            ...
            <Snackbar open={!!fcmNotif} autoHideDuration={6000} onClose={() => setFcmNotif(null)} anchorOrigin={{
      vertical: 'top',
      horizontal: 'right'
    }}>
                <Alert onClose={() => setFcmNotif(null)} severity="info" icon={false} sx={{
        width: '100%',
        maxWidth: 360,
        bgcolor: '#fff',
        color: '#333',
