...
export default function ReviewCard({
  ...
  const alreadyHelpful = review.helpfulBy?.some(id => id === authUser._id);
  const isOwner = authUser?.businessPeople === true && authUser?.businessName === business?.businessName;
  const handleHelpful = () => {
  ...
  return <div className={cx("review-card")}>
      ...
      <div className={cx("review-header")}>
        ...
        {review.isVerifiedUser && <VerifiedIcon fontSize="small" style={{
        color: "#1d9bf0",
        marginLeft: 6
      }} />}
      </div>
      ...
      <div className={cx("review-actions-row")}>
        ...
        <button className={cx(`review-action-btn ${alreadyHelpful ? "disabled" : ""}`)} disabled={alreadyHelpful} title={alreadyHelpful ? "You already marked this helpful" : "Mark helpful"} onClick={handleHelpful}>
