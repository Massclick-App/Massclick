...
const SexologistCards = () => {
  ...
  const sexologistList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading sexologist...</p>
      )}

      {!loading && sexologistList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Sexologist Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have sexologist listed right now.
          </p>
          <button
            className="go-home-button"
...
export default SexologistCards;
