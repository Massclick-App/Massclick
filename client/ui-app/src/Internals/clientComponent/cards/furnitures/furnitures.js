...
const FurnituresCards = () => {
  ...
  const furnituresList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading furnitures...</p>
      )}

      {!loading && furnituresList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Furniture Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have furnitures listed right now.
          </p>
          <button
            className="go-home-button"
...
export default FurnituresCards;
