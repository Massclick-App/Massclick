...
const PgAndHostelsCards = () => {
  ...
  const hostelList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading PG & Hostels...</p>
      )}

      {!loading && hostelList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No PG & Hostels Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have PG or hostel listings right now.
          </p>
          <button
            className="go-home-button"
