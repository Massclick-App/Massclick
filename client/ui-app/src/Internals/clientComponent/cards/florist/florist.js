...
const FloristCards = () => {
  ...
  const floristList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading florist...</p>
      )}

      {!loading && floristList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Florists Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have florist listed right now.
          </p>
          <button
            className="go-home-button"
