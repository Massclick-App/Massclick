...
const MatrimonyCards = () => {
  ...
  const matrimonyList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading matrimony...</p>
      )}

      {!loading && matrimonyList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Matrimony Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have matrimony listed right now.
          </p>
          <button
            className="go-home-button"
