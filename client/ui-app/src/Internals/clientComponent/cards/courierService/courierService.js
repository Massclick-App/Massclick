...
const CATEGORY = "courier service";
...
const CourierServiceCards = () => {
  ...
  const courierServiceList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading courier services...</p>
      )}

      {!loading && courierServiceList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Courier Services Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have courier services listed right now.
          </p>
          <button
            className="go-home-button"
      ...
      <div className="restaurants-list-wrapper">
        {courierServiceList.map((business) => {
          const averageRating =
