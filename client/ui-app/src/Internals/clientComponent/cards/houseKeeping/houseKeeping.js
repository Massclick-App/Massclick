...
const HouseKeepingCards = () => {
  ...
  const houseKeepingList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading HouseKeeping...</p>
      )}

      {!loading && houseKeepingList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Florists Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have houseKeeping listed right now.
          </p>
          <button
            className="go-home-button"
...
export default HouseKeepingCards;
