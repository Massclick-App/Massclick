...
const PhotographyCards = () => {
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading photographer...</p>
      )}

      {!loading && photographerList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Photographer Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have photographer listed right now.
          </p>
          <button
            className="go-home-button"
