...
const WeddingMallCards = () => {
  ...
  const weddingMallList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading weddingMall...</p>
      )}

      {!loading && weddingMallList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Wedding Mall Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have wedding mall listed right now.
          </p>
          <button
            className="go-home-button"
...
export default WeddingMallCards;
