...
const CATEGORY = "Security System";
...
const SecuritySystemCards = () => {
  ...
  const securitySystemList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading securitySystem...</p>
      )}

      {!loading && securitySystemList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No SecuritySystem Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have SecuritySystem listed right now.
          </p>
          <button
            className="go-home-button"
      ...
      <div className="restaurants-list-wrapper">
        {securitySystemList.map((business) => {
          const averageRating =
...
export default SecuritySystemCards;
