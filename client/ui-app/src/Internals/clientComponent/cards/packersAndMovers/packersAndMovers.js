...
const CATEGORY = "packers and movers";
...
const PackersAndMoversCards = () => {
  ...
  const packersAndMoversList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading packers and movers...</p>
      )}

      {!loading && packersAndMoversList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Packers and Movers Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have packers and movers listed right now.
          </p>
          <button
            className="go-home-button"
      ...
      <div className="restaurants-list-wrapper">
        {packersAndMoversList.map((business) => {
          const averageRating =
