...
const CategoryDynamicPage = () => {
  ...
  /* ========================================= */
  const realDistrict = unslugify(district);
  const realCategoryName = location.state?.categoryName || unslugify(categorySlug);
  const {
  ...
  useEffect(() => {
    if (realDistrict && realCategoryName) {
      dispatch(getBusinessByCategory(realCategoryName, realDistrict));
    }
  ...
  if (error && !businessList.length) {
    return <div className={cx("no-results-container")}>
        ...
        <button className={cx("go-home-button")} onClick={handleRetry}>
          Retry
        </button>
  ...
  return <>
      ...
      {loading && <p className={cx("loading-text")}>
      ...
      {!loading && businessList.length === 0 && <div className={cx("no-results-container")}>
