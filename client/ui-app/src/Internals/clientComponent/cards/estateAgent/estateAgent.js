...
const EstateAgentCards = () => {
  ...
  const estateAgentList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading Estate Agents...</p>
      )}

      {!loading && estateAgentList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Estate Agents Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have Estate Agents listed right now.
          </p>
          <button
            className="go-home-button"
...
export default EstateAgentCards;
