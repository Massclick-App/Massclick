...
const TransportsCards = () => {
  ...
  const transportsList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading Transports...</p>
      )}

      {!loading && transportsList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Transports Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have Transports listed right now.
          </p>
          <button
            className="go-home-button"
