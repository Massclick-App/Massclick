...
const CATEGORY = "catering service";
...
const CateringServiceCards = () => {
  ...
  const cateringServiceList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {!loading && cateringServiceList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No catering Services Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have catering services listed right now.
          </p>
          <button
            className="go-home-button"
      ...
      <div className="restaurants-list-wrapper">
        {cateringServiceList.map((business) => {
          const averageRating =
...
export default CateringServiceCards;
