...
const FencingCards = () => {
  ...
  const fencingList = categoryBusinessList[CATEGORY] || [];
  ...
  return (
    <>
      ...
      {loading && (
        <p className="loading-text">Loading fencing...</p>
      )}

      {!loading && fencingList.length === 0 && (
        <div className="no-results-container">
          <p className="no-results-title">No Fencing Found 😔</p>
          <p className="no-results-suggestion">
            We don’t have fencing listed right now.
          </p>
          <button
            className="go-home-button"
