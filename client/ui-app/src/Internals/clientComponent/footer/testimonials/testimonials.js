...
const TestimonialCard = ({
  testimonial
        ...
        <div className={cx("testimonial-author-info")}>
            <h4 className={cx("author-name")}>{testimonial.name}</h4>
            <p className={cx("author-source")}>
                <span className={cx("author-company")}>{testimonial.company}</span>
                <span className={cx("author-divider")}>|</span>
                {testimonial.source}
            </p>
        </div>
    </div>;
const Testimonials = () => {
  ...
  const cardDimensionsRef = useRef({
    cardWidth: 0,
    gap: 0
  });
  ...
  return <>
            ...
            <section className={cx("section-testimonials")}>
                <h2 className={cx("section-title-testimonials")}>Our Clients Say <span className={cx("highlight-text-testimonials")}>About Us</span></h2>

                <div className={cx("slider-container")}>
                    <div className={cx("slider-track")} ref={sliderRef} style={{
          transform: `translateX(0)`,
          transition: 'transform 1s ease-in-out'
        }}>
                        {clientTestimonials.map((testimonial, index) => <TestimonialCard key={index} testimonial={testimonial} />)}
                    </div>

                    <button className={cx("slider-control prev")} onClick={handlePrev} aria-label="Previous Testimonial">
                        <ChevronLeftIcon className={cx("control-icon")} />
                    </button>
                    <button className={cx("slider-control next")} onClick={handleNext} aria-label="Next Testimonial">
                        <ChevronRightIcon className={cx("control-icon")} />
                    </button>
                </div>

                <div className={cx("slider-indicators")}>
                    {clientTestimonials.map((_, index) => <button key={index} className={cx(`indicator-dot ${index === currentIndex ? 'active' : ''}`)} onClick={() => setCurrentIndex(index)} aria-label={`Go to slide ${index + 1}`} />)}
                </div>
            </section>
            <Footer />
        </>;
