...
const termsData = [{
  ...
  content: <p>a. These Terms shall be governed by and construed in accordance with the laws of India. Any disputes arising out of or relating to these Terms or the use of the Website shall be resolved exclusively by the courts located in Trichy.</p>
}, {
  id: 10,
  title: "10. Severability",
  content: <p>a. If any provision of these Terms is found to be invalid or unenforceable, the remaining provisions shall remain in full force and effect. The invalid or unenforceable provision shall be replaced by a valid and enforceable provision that comes closest to the intention of the original provision.</p>
}, {
  id: 11,
...
const AccordionItem = ({
  ...
  return <div className={cx(`accordion-item ${isOpen ? 'open' : ''}`)}>
            <button className={cx("accordion-header")} onClick={() => onClick(item.id)}>
                <h3 className={cx("accordion-title")}>{item.title}</h3>
                <div className={cx("accordion-icon")}>
  ...
  return <>
            ...
            <section className={cx("section-terms")}>
                <div className={cx("terms-header-wrapper")}>
                    ...
                    <p className={cx("section-subtitle-terms")}>
                ...
                <div className={cx("accordion-container")}>
                    {termsData.map(item => <AccordionItem key={item.id} item={item} isOpen={item.id === openItemId} onClick={handleToggle} />)}
                </div>
            </section>
            <Footer />
        </>;
