...
export default function AdvertisePage() {
  ...
  const handleSubmit = async e => {
    ...
    if (!formData.businessAddress) err.businessAddress = "Address required";
    setErrors(err);
    if (Object.keys(err).length === 0) {
      try {
        await dispatch(createAdvertise(formData));
        enqueueSnackbar(`${formData.businessName} submitted successfully!`, {
          variant: "success"
        });
        setFormData({
          businessName: "",
  ...
  return <div className={cx("advertise-wrapper")}>
      <div className={cx("advertise-card")}>
        <div className={cx("advertise-left")}>
        ...
        <div className={cx("advertise-right")}>
          ...
          <form className={cx("advertise-form")} onSubmit={handleSubmit}>
            <input type="text" placeholder="Business Name" value={formData.businessName} onChange={e => setFormData(p => ({
            ...p,
            businessName: e.target.value
          }))} />

            <input type="text" placeholder="Mobile Number" value={formData.mobileNumber} onChange={e => setFormData(p => ({
            ...p,
            mobileNumber: e.target.value
          }))} />

            <input type="text" placeholder="Pincode" value={formData.pincode} onChange={e => setFormData(p => ({
            ...p,
            ...
            position: "relative"
              ...
              <input type="text" value={formData.category} placeholder="Search category..." onChange={e => {
              ...
              {showCategorySuggest && searchCategory.length > 0 && <ul className={cx("category-suggest")}>
            ...
            <input type="text" placeholder="City" value={formData.city} onChange={e => setFormData(p => ({
            ...p,
            city: e.target.value
          }))} />

            <input type="text" placeholder="Business Address" value={formData.businessAddress} onChange={e => setFormData(p => ({
            ...p,
