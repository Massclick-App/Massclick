...
const normalizeBusiness = (business = {}) => {
  const normalized = editableFields.reduce(
    (acc, key) => ({ ...acc, [key]: business[key] ?? initialFormData[key] }),
    {}
  );
...
export default function EditBusinessPage() {
  ...
  const { searchCategory = [], loading: categoryLoading } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(findBusinessByMobile(mobileNumber)).then((business) => {
      ...
      setKycDocumentPreviews(
        (business.kycDocuments || []).map((url, index) => ({
  ...
  useEffect(() => {
    ...
    setKycDocumentPreviews(
      (matchedBusiness.kycDocuments || []).map((url, index) => ({
        name: `Existing document ${index + 1}`,
        preview: url,
        type: "existing",
        value: url,
  ...
  const updateOpeningHour = (index, field, value) => {
    setFormData((prev) => ({
      ...prev,
      openingHours: prev.openingHours.map((hour, hourIndex) =>
  ...
  const renderInput = ({ field, label, type = "text", required = false }) => (
    <div className="edit-business-field" key={field}>
      <label htmlFor={field}>
        {label}
        {required ? " *" : ""}
      </label>
    ...
    <div className="edit-business-field">
      ...
      <Autocomplete
        ...
        renderOption={(props, option) => (
          <li {...props} key={option._id || option.category}>
            <div className="edit-business-category-option">
      ...
      />
      <p className="edit-business-help-text">
  ...
  const renderStep = () => {
    if (activeStep === 0) {
      return (
        <div className="edit-business-grid">
          {renderInput({ field: "businessName", label: "Business Name", required: true })}
          {renderCategoryAutocomplete()}
          {renderInput({ field: "subcategory", label: "Subcategory" })}
          {renderInput({ field: "location", label: "Location", required: true })}
          {renderInput({ field: "plotNumber", label: "Plot / Door Number" })}
          {renderInput({ field: "street", label: "Street / Area" })}
          {renderInput({ field: "pincode", label: "Pincode" })}
          {renderInput({ field: "globalAddress", label: "Full Address" })}
          {renderInput({ field: "contact", label: "Primary Contact", required: true })}
          {renderInput({ field: "contactList", label: "Enquiry Number" })}
          {renderInput({ field: "whatsappNumber", label: "WhatsApp Number" })}
          {renderInput({ field: "email", label: "Business Email", type: "email" })}
          {renderInput({ field: "gstin", label: "GSTIN" })}
          {renderInput({ field: "experience", label: "Experience" })}
        </div>
      );
    }
    ...
    if (activeStep === 1) {
      return (
        <>
          <div className="edit-business-grid">
            {renderInput({ field: "website", label: "Website" })}
            {renderInput({ field: "googleMap", label: "Google Map Link" })}
            {renderInput({ field: "facebook", label: "Facebook" })}
            {renderInput({ field: "instagram", label: "Instagram" })}
            {renderInput({ field: "youtube", label: "YouTube" })}
            {renderInput({ field: "pinterest", label: "Pinterest" })}
            {renderInput({ field: "twitter", label: "Twitter / X" })}
            {renderInput({ field: "linkedin", label: "LinkedIn" })}
          </div>

          <div className="edit-business-field edit-business-full">
            <label htmlFor="restaurantOptions">Restaurant / Hotel Option</label>
            <select
              id="restaurantOptions"
              value={formData.restaurantOptions || ""}
              onChange={(event) => updateField("restaurantOptions", event.target.value)}
            >
          ...
          <div className="edit-business-upload">
            <div>
    ...
    if (activeStep === 2) {
      return (
        <div className="edit-business-kyc">
          <div className="edit-business-upload edit-business-upload-compact">
            <div>
          ...
          <div className="edit-business-kyc-list">
            {kycDocumentPreviews.length === 0 ? (
              <div className="edit-business-loading">No KYC documents uploaded.</div>
            ) : (
              kycDocumentPreviews.map((doc, index) => (
                <div className="edit-business-kyc-item" key={`${doc.name}-${index}`}>
                  <div>
                  ...
                  {doc.type === "existing" ? (
                    <Chip label="Saved document" size="small" />
                  ) : (
                    <Button variant="outlined" color="error" onClick={() => removeKycDocument(index)}>
                      Remove
                    </Button>
                  )}
                </div>
    ...
    if (activeStep === 3) {
      return (
        <div className="edit-business-hours">
          {formData.openingHours.map((hour, index) => (
            <div className="edit-business-hour-row" key={hour.day || index}>
              <strong>{hour.day}</strong>
              <input
                type="time"
                value={hour.is24Hours ? "00:00" : hour.open || "09:00"}
                disabled={hour.isClosed || hour.is24Hours}
                onChange={(event) => updateOpeningHour(index, "open", event.target.value)}
              />
              <input
                type="time"
                value={hour.is24Hours ? "23:59" : hour.close || "18:00"}
                disabled={hour.isClosed || hour.is24Hours}
                onChange={(event) => updateOpeningHour(index, "close", event.target.value)}
              />
              <select
                value={hour.isClosed ? "closed" : hour.is24Hours ? "24hours" : "open"}
                onChange={(event) => {
    ...
    return (
      <div className="edit-business-grid">
        <div className="edit-business-field edit-business-full">
          ...
          <div className="edit-business-keywords-box">
            <div className="edit-business-keyword-chips">
              {(formData.keywords || []).map((keyword) => (
                <button
                  type="button"
                  className="edit-business-keyword-chip"
                  key={keyword}
            ...
            <div className="edit-business-keyword-entry">
              <input
                id="keywords"
                value={keywordInput}
        ...
        {renderInput({ field: "title", label: "Display Title" })}
        {renderInput({ field: "slug", label: "Slug" })}
        <div className="edit-business-field edit-business-full">
        ...
        {renderInput({ field: "seoTitle", label: "SEO Title" })}
        <div className="edit-business-field edit-business-full">
    ...
    <>
      ...
      <div className="edit-business-page">
        <div className="edit-business-container">
          ...
          {matchedBusinessLoading ? (
            <div className="edit-business-loading">Loading your business details...</div>
          ) : !businessId ? (
            <div className="edit-business-loading">
              No business listing was found for this login number.
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <Stepper activeStep={activeStep} alternativeLabel className="edit-business-stepper">
                {steps.map((label) => (
                  <Step key={label}>
              ...
              <div className="edit-business-step-content">{renderStep()}</div>

              <Box className="edit-business-actions">
                ...
                {activeStep < steps.length - 1 ? (
                  ...
                  <Button variant="contained" type="submit" disabled={savingBusiness}>
