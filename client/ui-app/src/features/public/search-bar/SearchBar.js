...
import styles from "features/public/search-bar/searchBar.module.css";
import AddBusinessModel from "features/public/auth/AddBusinessModal.js";
import { Box, Button,
...
const SearchBar = ({
  ...
  return <header className={cx(`search-header ${isVisible ? "visible" : ""}`)} style={{
    backdropFilter: isVisible ? "blur(8px)" : "none"
            ...
            <div className={cx("search-header-content")}>
                ...
                <div className={cx("logo-section")}>
                    <div className={cx("logo-circle")}>
                        <img src="/apple-touch-icon.png" alt="Logo" className={cx("logo-image")} />
                    </div>
                    <Box sx={{
          display: "flex",
            ...
            mt: 0.5
                ...
                <div className={cx("search-area")}>
                    <div className={cx("search-input location")}>
                        <i className={cx("fa-solid fa-location-dot icon start-icon")}></i> 
                        <input type="text" placeholder="Location" value={locationName} onChange={e => setLocationName(e.target.value)} list="locationOptions" />
                        <datalist id="locationOptions">
                            {locationOptions.map((opt, i) => <option key={i} value={opt.label} />)}
                        </datalist>
                    </div>

                    <div className={cx("search-input main")}>
                        <input type="text" placeholder="Search for ..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSearch()} />
                        <MicIcon className={cx("input-adornment end")} />
                    </div>


                    <button className={cx("search-btn")} onClick={handleSearch}>
                        <span>Search</span> <i className={cx("fa-solid fa-magnifying-glass")}></i>
                    </button>
      ...
      sx={{
        ...
        // HIDE on XS, SM, SHOW on MD and up
