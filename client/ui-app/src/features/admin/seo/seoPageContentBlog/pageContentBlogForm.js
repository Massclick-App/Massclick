...
const renderTableBlockHtml = (block = {}) => {
  ...
  const renderRow = (row, tag) =>
    `<tr>${row.cells.map((cell) => `<${tag}>${escapeHtml(cell)}</${tag}>`).join("")}</tr>`;
  const caption = table.caption ? `<caption>${escapeHtml(table.caption)}</caption>` : "";
  const head = table.hasHeaderRow ? `<thead>${renderRow(rows[0], "th")}</thead>` : "";
    ...
    <form className={cx("seo-form")} onSubmit={handleSubmit}>
      ...
      <section className={cx("editor-card", "premium-card")}>
        ...
        {!previewMode ? (
          ...
          <div className={cx("preview-content")} dangerouslySetInnerHTML={{ __html: renderInlineTables(formData.pageContent, contentBlocks) }} />
        )}
      </section>
      ...
      <section className={cx("editor-card", "premium-card")}>
        <div className={cx("content-blocks-header")}>
          <div>
            ...
            <p className={cx("section-helper")}>Type {"{table1}"} in Page Content where the first table should appear.</p>
          </div>
              ...
              <div key={block.id} className={cx("content-block")}>
                <div className={cx("block-header")}>
                  <div>
                    ...
                    <p className={cx("block-helper")}>Use {idx === 0 ? "{table1} or {table}" : `{table${idx + 1}}`} in Page Content.</p>
                  </div>
