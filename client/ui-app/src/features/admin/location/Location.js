...
import { getAllLocation, createLocation, editLocation, deleteLocation } from "state/actions/locationAction.js";
import styles from "features/admin/location/location.module.css";
...
export default function Location() {
  ...
  const confirmDelete = () => {
    debugger;
    if (selectedRow?.id) {
      dispatch(deleteLocation(selectedRow.id)).then(() => {
        dispatch(getAllLocation());
  ...
  const rows = location.filter(loc => loc.isActive).map((loc, index) => ({
    id: loc._id || index,
    country: loc.country,
    state: loc.state,
  ...
  return <div className={cx("location-page")}>
            ...
            <div className={cx("location-card location-form-section")}>
                ...
                <form onSubmit={handleSubmit} className={cx("location-form-grid")}>
                    {fields.map(({
          label,
          name
                    ...
                    <div className={cx("location-form-input-group location-col-span-all location-upload-section")}>
                        <div className={cx("location-upload-content")}>
                            <button type="submit" className={cx("location-submit-button")} disabled={loading}>
                                {loading ? <CircularProgress size={24} color="inherit" /> : editingId ? "Update Location" : "Create Location"}
                            </button>

                            {editingId && <button type="button" className={cx("location-upload-button")} onClick={resetForm}>
                                    Cancel
                                </button>}
                        </div>
                    </div>
                ...
                {error && <p className={cx("location-error-text")} style={{
        marginTop: "16px"
      }}>
                        {(() => {
        ...
        width: "100%"
                    ...
                    <CustomizedTable data={rows} columns={locationList} total={total} fetchData={(pageNo, pageSize, options) => dispatch(getAllLocation({
          pageNo,
          pageSize,
          options
        }))} />
