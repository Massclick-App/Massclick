...
import { isValidEmailAddress } from "shared/utils/emailValidation.js";
import { getAllBusinessList, createBusinessList, editBusinessList, editBusinessSection, deleteBusinessList, trackQrDownload, updateBusinessBadges, regenerateBusinessCertificates, exportBusinessList, revertPaidStatus } from "state/actions/businessListAction.js";
import { getAllLocation, createLocation } from "state/actions/locationAction.js";
import { getAllCategory, businessCategorySearch } from "state/actions/categoryAction.js";
import { getAllUsersClient, getUserClientSuggestion } from "state/actions/userClientAction.js";
...
import BusinessFormStep0 from "features/admin/business/components/BusinessFormStep0.js";
import BusinessFormStep1 from "features/admin/business/components/BusinessFormStep1.js";
import BusinessFormStep2 from "features/admin/business/components/BusinessFormStep2.js";
import BusinessSidebar from "features/admin/business/components/BusinessSidebar.js";
import LogoCropperModal from "features/admin/business/components/LogoCropperModal.js";
import { downloadBusinessDetailsTemplate } from "features/admin/business/businessDetailsTemplate.js";
...
const BusinessList = React.memo(() => {
  ...
  // Gallery order is just array order, all the way through: the section sends
  // the stored photos as retainedBusinessImages in this order, the server keeps
  // that order in businessImagesKey, and both the website and the app render
  // businessImages as they receive it.
  ...
  const handleReorderStoredGalleryImage = (from, to) => {
    setFormData(prev => ({
      ...prev,
      businessImages: moveArrayItem(prev.businessImages || [], from, to)
    }));
  };
  const handleReorderGalleryFile = (from, to) => {
    setGalleryFiles(prev => moveArrayItem(prev, from, to));
  };
  ...
  const getBusinessValidationRules = (data, {
    ...
    return [
      ...
      { field: "email", label: "Email", required: false, example: "contact@organisation.org", suggestion: "All valid domains are supported, including .org, .in and .co.in.", validate: value => isValidEmailAddress(value) || "must be a valid email address with a complete domain." },
      { field: "contact", label: "Phone", required: false, example: "9876543210", suggestion: "Enter the main customer-facing number.", validate: value => passOrMessage(digitsOnly(value).length >= 10 && digitsOnly(value).length <= 15, "must contain 10 to 15 digits.") },
      { field: "contactList", label: "Enquiry number", required: false, example: "9876543210", suggestion: "Use the number that should receive enquiries.", validate: value => passOrMessage(digitsOnly(value).length >= 10 && digitsOnly(value).length <= 15, "must contain 10 to 15 digits.") },
      { field: "whatsappNumber", label: "WhatsApp number", required: false, example: "9876543210", suggestion: "Use a WhatsApp-enabled number.", validate: value => passOrMessage(digitsOnly(value).length >= 10 && digitsOnly(value).length <= 15, "must contain 10 to 15 digits.") },
      ...
      case 0:
        return (
          <BusinessFormStep0
            ...
            handleReorderStoredGalleryImage={handleReorderStoredGalleryImage}
            handleReorderGalleryFile={handleReorderGalleryFile}
