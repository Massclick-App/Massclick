...
import { getAllUsers, createUser, editUser, deleteUser } from "state/actions/userAction.js";
import { getAllRoles } from "state/actions/rolesAction.js";
import { Box, Button, CircularProgress, Typography, Dialog, DialogTitle, DialogContent, DialogActions, Avatar } from "@mui/material";
import styles from "features/admin/users/user.module.css";
...
export default function User() {
  ...
  const validateForm = () => {
    ...
    if (!formData.emailId) newErrors.emailId = "Email is required";else if (!/\S+@\S+\.\S+/.test(formData.emailId)) newErrors.emailId = "Invalid email format";
    if (!formData.role) newErrors.role = "Role is required";
    if (!formData.businessLocation) newErrors.businessLocation = "Business Location is required";
    if (!formData.businessCategory) newErrors.businessCategory = "Business Category is required";
    if (formData.role === "SalesOfficer" && !formData.managedBy) newErrors.managedBy = "Assigned Manager is required for Sales Officer";
    setErrors(newErrors);
  ...
  return <div className={cx("user-page")}>
      ...
      {activeView === "form" && <div className={cx("user-card form-section")}>
        ...
        {error && <p className={cx("user-error-text")} style={{
        marginTop: "16px"
      }}>
            {(() => {
