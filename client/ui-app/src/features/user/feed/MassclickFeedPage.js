...
const spotlightRoutes = {
  ...
  Events: "/events",
  Businesses: "/",
  Messages: "/user-customer-service",
  Saved: "/user_favorites",
  Followers: "/user_dashboard",
  Insights: "/user_dashboard",
  Reports: "/user_dashboard",
  Subscriptions: "/user_dashboard",
};
const createTypes = [
  ["◇", "Offer / Discount"],
  ["▤", "Update / News"],
...
const spotlightTools = [
  ["□", "Post Scheduler", "Schedule posts in advance"],
  ["▤", "Templates", "Ready to use designs"],
  ["▣", "Drafts", "Continue your drafts"],
  ["▧", "Media Library", "Your images & videos"],
  ["⚑", "Ad Boost", "Promote your post"],
  ["▢", "Leads Inbox", "Manage your leads"],
  ["⌘", "QR Code", "Share your post"],
  ["▥", "Post Reports", "Performance analytics"],
];
...
const ReplicaPost = ({
  ...
  const runAction = async (name, action, successMessage = "") => {
    ...
    try {
      ...
      setActionNotice("That action could not be completed. Please try again.");
      return false;
    } finally {
      setBusyAction("");
    }
  ...
  const submitComment = () => {
    if (comment.trim())
      runAction("comment", () => onComment(post._id, comment.trim())).then(
        (saved) => {
          if (saved) {
            setComment("");
            setCommenting(false);
  ...
  return (
    <article className={cx("replica-post")}>
      <header>
        <button
          className={cx("replica-profile-trigger")}
          type="button"
          onClick={() => onProfile(post)}
          aria-label={`View ${business} profile`}
        >
          <span
            className={cx("replica-avatar", index % 2 && "replica-avatar-warm")}
          >
            {initials}
          </span>
          <span>
            ...
            <small>
              ...
              · Public
              {post?.businessLocation ? ` · ${post.businessLocation}` : ""}
            </small>
          </span>
        </button>
        {post.businessId && (
          <button
            className={cx("replica-follow", post.isFollowing && "following")}
            type="button"
            aria-pressed={Boolean(post.isFollowing)}
            onClick={() => onFollow(post.businessId, post.isFollowing)}
          >
            {post.isFollowing ? "Following" : "Follow"}
          </button>
        )}
        <button
          className={cx("replica-more")}
          type="button"
          onClick={() => onOpen(post)}
      ...
      {media &&
        (media.mediaType === "video" ? (
          <video
            className={cx("replica-post-image")}
            src={media.mediaUrl}
            controls
            preload="metadata"
          />
          ...
          <img
            className={cx("replica-post-image")}
            src={media.mediaUrl}
            alt={media.fileName || business}
            onClick={() => onOpen(post)}
          />
        ))}
      <div className={cx("poster-facts")}>
        <span>
          <Star size={16} />
          <b>4.8</b>
        ...
        <span>
          <MapPin size={16} />
          <b>{post?.businessLocation || "Local"}</b>
      ...
      <div className={cx("poster-metrics")}>
        <span>
          <Eye />
          <b>{formatCompactNumber(post?.viewsCount || 0)}</b>
          <small>Views</small>
        </span>
        <span className={post.likedByMe ? cx("metric-liked") : ""}>
        ...
        <span>
          <MessageSquareText />
          <b>{(post?.enquiriesCount || 0).toLocaleString()}</b>
          <small>Enquiries</small>
        </span>
        <span>
          <Share2 />
          <b>{shares.toLocaleString()}</b>
          <small>Shares</small>
        </span>
      </div>
      <div className={cx("poster-ctas")}>
        <button
          type="button"
          disabled={busyAction === "enquiry"}
          onClick={() =>
            runAction(
              "enquiry",
              () => onEnquire(post._id),
        ...
        >
          <MessageSquareText />
          {busyAction === "enquiry" ? "Sending…" : "Enquire Now"}
        </button>
        {(post.callToActions || []).slice(0, 2).map((item) => (
          <a
            key={item.action}
            href={getActionHref(item)}
            target={
              ["call", "whatsapp"].includes(item.action) ? undefined : "_blank"
            }
            rel="noreferrer"
          >
            {item.action === "call" ? <Phone /> : <MapPin />}
            {item.label}
          </a>
        ))}
        {!post.callToActions?.length && (
          ...
          >
            <Phone />
            {busyAction === "callback" ? "Requesting…" : "Request Callback"}
          </button>
        )}
      </div>
      {actionNotice && (
        <div className={cx("poster-notice")} role="status">
          ✓ {actionNotice}
        </div>
      )}
      <div className={cx("replica-social-counts")}>
        <span>
          <i>●</i>
          <b>♥</b> {likes.toLocaleString()}
        </span>
        <span>
          {comments.toLocaleString()} Comment{comments === 1 ? "" : "s"} ·{" "}
      ...
      <footer>
        <button
          ...
          className={post.likedByMe ? cx("liked") : ""}
          onClick={() => runAction("like", () => onLike(post._id))}
        >
        ...
        <button type="button" onClick={() => setCommenting((value) => !value)}>
        ...
        <button
          ...
          className={post.savedByMe ? cx("saved") : ""}
          onClick={() => runAction("save", () => onSave(post._id))}
        >
      ...
      {commenting && (
        <div className={cx("replica-comment")}>
          <input
            autoFocus
            maxLength={500}
            value={comment}
          ...
          <button
            type="button"
            disabled={!comment.trim() || busyAction === "comment"}
...
function BusinessProfileModal({ post, posts, onClose, onFollow }) {
  ...
  const businessPosts = posts.filter(
    (item) => String(item.businessId || "") === String(post?.businessId || ""),
  );
  const publicActions = (post?.callToActions || [])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <header>
          <div className={cx("profile-avatar")}>{initials}</div>
          <div>
            <small>
              {post?.ownerActorType === "business"
                ? "Business account"
                : "Business person"}
            </small>
            <h2 id="business-profile-title">{business}</h2>
            <p>
              {[post?.businessCategory, post?.businessLocation]
                .filter(Boolean)
                .join(" · ") || "Local business on MassClick"}
            </p>
          </div>
        </header>
        <div className={cx("profile-stats")}>
          <span>
            <strong>{(post?.followersCount || 0).toLocaleString()}</strong>
            <small>Followers</small>
          </span>
          <span>
            <strong>{businessPosts.length}</strong>
            <small>Spotlight Posts</small>
          </span>
          <span>
            <strong>
              {businessPosts
                .reduce((total, item) => total + (item.likesCount || 0), 0)
                .toLocaleString()}
            </strong>
            <small>Interested</small>
          </span>
        ...
        <div className={cx("profile-details")}>
          ...
          <p>
            ...
            <span>
              ...
              <strong>{post?.businessCategory || "Local business"}</strong>
            </span>
          </p>
          <p>
            <MapPin />
            <span>
              ...
              <strong>
                {post?.businessLocation || "Location not provided"}
              </strong>
            </span>
          </p>
          <p>
          ...
          <div className={cx("profile-contact")}>
            ...
            {publicActions.map((item) => (
              <a
                ...
                href={getActionHref(item)}
                target={
                  ["call", "whatsapp"].includes(item.action)
                    ? undefined
                    : "_blank"
                }
                rel="noreferrer"
        ...
        {post?.businessId && (
          <footer>
            <button
              type="button"
              className={post.isFollowing ? cx("following") : ""}
              aria-pressed={Boolean(post.isFollowing)}
              onClick={() => onFollow(post.businessId, post.isFollowing)}
            >
              {post.isFollowing ? "✓ Following" : "+ Follow Business"}
            </button>
            <small>
              {post.isFollowing
                ? "You’ll see this business’s updates first."
                : "Follow to see new posts and offers first."}
            </small>
          </footer>
        )}
      </section>
...
function BusinessFriendModal({ business, onClose, onFollow }) {
  ...
  const initials = business.businessName
    ...
    .join("")
    .toUpperCase();
  const profilePath = buildBusinessPath({
  ...
  return (
    ...
    >
      <section
        className={cx("profile-modal", "friend-preview-modal")}
        role="dialog"
        aria-modal="true"
        aria-labelledby="friend-preview-title"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button
          className={cx("profile-close")}
        ...
        <header>
          <div className={cx("profile-avatar")}>
            {business.businessLogoUrl ? (
              <img src={business.businessLogoUrl} alt="" />
            ) : (
              initials
            )}
          </div>
          <div>
            ...
            <h2 id="friend-preview-title">{business.businessName}</h2>
            <p>
              {[business.businessCategory, business.businessLocation]
                .filter(Boolean)
                .join(" · ") || "Local business on MassClick"}
            </p>
          </div>
        ...
        <div className={cx("profile-stats")}>
          ...
          <span>
            <strong>{business.postsCount || 0}</strong>
            <small>Spotlight Posts</small>
          </span>
          <span>
            <strong>
        ...
        <div className={cx("profile-details")}>
          ...
          <p>
            ...
            <span>
              <small>Category</small>
              <strong>{business.businessCategory || "Local business"}</strong>
            </span>
          </p>
          <p>
            <MapPin />
            <span>
              <small>Location</small>
              <strong>
                {business.businessLocation || "Location not provided"}
              </strong>
            </span>
          </p>
          <p>
            <UserRound />
            <span>
              <small>MassClick connection</small>
              <strong>
                {business.isFollowing
                  ? "You are following this business"
                  : "Available to follow"}
              </strong>
            </span>
          </p>
        </div>
        <footer className={cx("friend-preview-actions")}>
          <button
            type="button"
            className={business.isFollowing ? cx("following") : ""}
            onClick={() => onFollow(business.businessId, business.isFollowing)}
          >
            {business.isFollowing ? "✓ Following" : "+ Follow Business"}
          </button>
          <button type="button" onClick={() => navigate(profilePath)}>
            View Full Profile →
          </button>
          <small>
            The full profile opens as a separate business details page.
          </small>
        </footer>
      </section>
    </div>
...
function StoryEditor({ initialStory, busy, error, onClose, onPublish }) {
  const [draft, setDraft] = useState(() =>
    initialStory
      ? {
          ...
          ...initialStory.storyStyle,
        }
      : storyDefaults,
  );
  const [preview, setPreview] = useState(
    initialStory?.mediaItems?.[0]?.mediaUrl || "",
  );
  const [previewType, setPreviewType] = useState(
    initialStory?.mediaItems?.[0]?.mediaType || "",
  );
  ...
  return (
    <div
      className={cx("story-editor-backdrop")}
      role="presentation"
      onMouseDown={onClose}
    >
      <section
        className={cx("story-editor")}
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <small>Visible for 24 hours</small>
            <h2 id="story-editor-title">
              {initialStory ? "Edit your story" : "Create your story"}
            </h2>
          </div>
        ...
        <div className={cx("story-editor-layout")}>
          <div
            className={cx("story-canvas")}
            style={{
              background: draft.background,
          ...
          >
            ...
            <strong className={cx(`story-font-${draft.font}`)}>
              {draft.text || "Your story text"}
            </strong>
            {(draft.musicTitle || draft.musicArtist) && (
              <span>
                ♫ {draft.musicTitle || "Music"}
                {draft.musicArtist ? ` · ${draft.musicArtist}` : ""}
              </span>
            )}
          </div>
          <div className={cx("story-controls")}>
            <label>
              Story text
              <textarea
                maxLength={1200}
                ...
                placeholder="Share an update, offer or moment…"
              />
              <small>{draft.text.length}/1200</small>
            </label>
            {!initialStory && (
              <label className={cx("story-upload")}>
                Photo or video
                <input
                  type="file"
                ...
                />
                <span>
                  <Picture /> Choose media
                </span>
              </label>
            )}
            <fieldset>
              <legend>Background</legend>
              <div className={cx("story-swatches")}>
                {storyBackgrounds.map((color) => (
                  <button
                    ...
                    onClick={() => setDraft({ ...draft, background: color })}
                  />
                ))}
              </div>
            </fieldset>
            <div className={cx("story-control-row")}>
              <label>
                Text color
                <input
                  type="color"
              ...
              <label>
                ...
                <select
                  value={draft.font}
                  onChange={(event) =>
                    setDraft({ ...draft, font: event.target.value })
                  }
                >
              ...
              <label>
                ...
                <select
                  value={draft.align}
                  onChange={(event) =>
                    setDraft({ ...draft, align: event.target.value })
                  }
                >
            ...
            <div className={cx("story-music-fields")}>
              <label>
                Music title
                <input
                  maxLength={80}
                  ...
                  onChange={(event) =>
                    setDraft({ ...draft, musicTitle: event.target.value })
                  }
                  placeholder="Licensed track title"
                />
              </label>
              <label>
                Artist
                <input
                  ...
                  onChange={(event) =>
                    setDraft({ ...draft, musicArtist: event.target.value })
                  }
                  placeholder="Artist name"
                />
              </label>
            </div>
            <p className={cx("story-editor-note")}>
              Music is attribution metadata; playback needs a licensed music
              provider.
            </p>
            {error && (
              <p className={cx("story-editor-error")} role="alert">
                {error}
              </p>
            )}
            <footer>
              ...
              <button
                ...
                disabled={
                  busy ||
                  (!draft.text.trim() && !draft.mediaFiles.length && !preview)
                }
                onClick={publish}
              >
                {busy
...
const SpotlightReplica = ({
  ...
  const displayPosts = (
    savedOnly ? filteredPosts.filter((post) => post.savedByMe) : filteredPosts
  ).slice(0, savedOnly ? 50 : 4);
  ...
  const stories = posts
    .filter(
      (post) =>
        ...
        post.status === "active" &&
        !post.isDeleted &&
        storyClock - new Date(post.createdAt).getTime() < 86400000,
    )
    .map((post) => ({
      name: post.businessName || "MassClick user",
      post,
      image: post.mediaItems?.[0]?.mediaUrl || "",
      mediaType: post.mediaItems?.[0]?.mediaType || "",
      mine:
        Boolean(post.ownedByMe) ||
        (Boolean(currentUserId) && String(post.ownerUserId) === currentUserId),
    }));
  const showStory = (index) =>
    setSelectedStory(Math.max(0, Math.min(stories.length - 1, index)));
  const submitStory = async (payload) => {
    setStoryBusy(true);
    setStoryError("");
    try {
      if (storyEditor) await editStory(storyEditor._id, payload);
  ...
  const removeSelectedStory = async (story) => {
    ...
    try {
      ...
      setStoryError(
        error?.response?.data?.message || "Story could not be deleted.",
      );
    } finally {
      setStoryBusy(false);
    }
  ...
  useEffect(() => {
    if (!businessSearchReady.current) {
      businessSearchReady.current = true;
      return undefined;
    }
    const timer = window.setTimeout(
      () => searchBusinesses(businessSearch.trim()),
      350,
    );
  ...
  const types = [
    [Tag, "Offer / Discount"],
    [Newspaper, "Update / News"],
    [CalendarDays, "Event"],
    [LayoutGrid, "Product"],
    [BellRing, "Service"],
    [Send, "Announcement"],
    [BriefcaseBusiness, "Job / Hiring"],
    [BarChart3, "Poll / Survey"],
    [Play, "Video Post"],
  ];
  return (
    <>
      <StickySearchBar />
      <main className={cx("replica-page")}>
        <aside className={cx("replica-sidebar")}>
          <nav>
            {replicaMenu.map(([Icon, label], index) => (
              <button
                key={label}
                type="button"
                className={
                  (label === "Saved" ? savedOnly : index === 0 && !savedOnly)
                    ? cx("active")
                    : ""
                }
                onClick={
              ...
              >
                ...
                <span>{label}</span>
                {label === "Saved" && (
                  <b className={cx("saved-count")}>{savedCount}</b>
                )}
              </button>
            ))}
          </nav>
          <section>
            ...
            <h3>Go Premium</h3>
            <p>
              Unlock advanced tools, analytics and more to grow your business.
            </p>
            <button type="button">Upgrade Now →</button>
          </section>
        </aside>
        <div className={cx("replica-center")}>
          <section className={cx("replica-stories")}>
            <header>
              <h2>Stories & Status</h2>
              {canStory && (
                <button type="button" onClick={() => setStoryEditor(false)}>
                  Create status
                </button>
              )}
            </header>
            <div>
              {canStory && (
                <button type="button" onClick={() => setStoryEditor(false)}>
                  <span className={cx("story-ring", "story-add")}>+</span>
              ...
              {stories.map((story, index) => (
                ...
                >
                  <span className={cx("story-ring")}>
                    {story.image && story.mediaType === "image" ? (
                  ...
                  <small>{story.mine ? "Your Story" : story.name}</small>
                </button>
              ))}
              {!canStory && !stories.length && (
                <p className={cx("replica-muted")}>
                  No active stories right now.
                </p>
              )}
            </div>
          </section>
          <section className={cx("replica-composer")}>
            <span className={cx("replica-avatar")}>MC</span>
            <button type="button" onClick={openComposer}>
              What’s on your mind today? <span>☺</span>
            </button>
            <div>
              {[
                [Picture, "Photo / Video"],
                ...
                <button type="button" key={label} onClick={openComposer}>
                  <Icon size={17} />
                  {label}
                </button>
              ))}
          ...
          {savedOnly && (
            <section className={cx("saved-heading")}>
              <div>
                ...
                <span>
                  ...
                  <small>
                    ...
                    posts, posters, videos and documents
                  </small>
                </span>
              </div>
          ...
          <div className={cx("replica-filters")} id="spotlight-posts">
            {[
              "All Posts",
              "Offers",
              "Updates",
              ...
              const filter = label === "All Posts" ? "All" : label;
              return (
                <button
                  type="button"
                  key={label}
                  className={activeFilter === filter ? cx("active") : ""}
          ...
          <section className={cx("replica-feed")}>
            {displayPosts.length ? (
              displayPosts.map((post, index) => (
                <ReplicaPost
                  ...
                  onProfile={(item) => setProfilePostId(item._id)}
                  onOpen={openPost}
                />
              ))
            ) : (
              <div className={cx("replica-empty")}>
                {savedOnly ? <Bookmark /> : <Newspaper />}
                <strong>
                  {savedOnly ? "No saved posts yet" : "No Spotlight posts yet"}
                </strong>
                <span>
                  {savedOnly
                    ? "Tap Save on any post, poster, video or document to keep it here."
                    : "Create the first post to start your local feed."}
                </span>
                <button
                  type="button"
                  onClick={savedOnly ? () => setSavedOnly(false) : openComposer}
                >
                  {savedOnly ? "Browse Spotlight" : "Create Post"}
                </button>
              </div>
            )}
          </section>
        ...
        <aside className={cx("replica-right")}>
          <section className={cx("replica-insights")}>
            <header>
              <h2>Spotlight Insights</h2>
              <button type="button" onClick={() => moveToSection("Insights")}>
                View All
              </button>
            </header>
            <div>
              {[
                [Eye, "Views", totalViews],
                [Heart, "Engagement", totalEngagements],
                [UserRound, "Leads", totalLeads],
                [Newspaper, "Posts", posts.length],
              ].map(([Icon, label, value]) => (
                <button
                  type="button"
                  key={label}
                  onClick={() => moveToSection("Insights")}
                >
                  <Icon size={18} />
                  <small>{label}</small>
            ...
            >
              <path d="M5 125 C45 115 55 72 95 76 S145 88 170 38 S225 23 245 65 S280 88 295 82" />
              <path
                className={cx("orange")}
                d="M5 140 C50 132 62 102 100 108 S150 112 178 78 S220 58 246 94 S275 112 295 110"
              />
            </svg>
            <footer>
              <span>● Views</span>
          ...
          <section className={cx("business-friends")}>
            <header>
              <div>
                <h2>Find Business Friends</h2>
                <p>Follow local businesses to see their posts first.</p>
              </div>
              <small>
                {businessLoading
                  ? "Searching…"
                  : `${businessTotal} result${businessTotal === 1 ? "" : "s"}`}
              </small>
            </header>
            <label>
              <span>⌕</span>
              <input
                ...
                aria-label="Search all business friends"
              />
            </label>
            <div>
              {visibleBusinesses.length ? (
                visibleBusinesses.map((business) => (
                  <article key={business.businessId}>
                    <button
                      className={cx("business-friend-profile")}
                      type="button"
                    ...
                    >
                      ...
                      <div>
                        ...
                        <em>
                          {business.followersCount} follower
                          {business.followersCount === 1 ? "" : "s"} ·{" "}
                          {business.postsCount} post
                          {business.postsCount === 1 ? "" : "s"}
                        </em>
                      </div>
                    </button>
                    <button
                      type="button"
                      className={business.isFollowing ? cx("following") : ""}
                      aria-pressed={Boolean(business.isFollowing)}
                      onClick={() =>
                        handleFollow(business.businessId, business.isFollowing)
                      }
                    >
            ...
            <nav
              className={cx("business-pagination")}
              aria-label="Business friends pages"
            >
              <button
                type="button"
                disabled={businessLoading || businessPage === 1}
                onClick={() => loadBusinessPage(businessPage - 1)}
              >
                ← Previous
              </button>
              <span>
                {businessLoading ? "Loading…" : `Page ${businessPage}`}
              </span>
              <button
                type="button"
                disabled={businessLoading || !businessHasMore}
                onClick={() => loadBusinessPage(businessPage + 1)}
              >
                Next →
              </button>
          ...
          <section className={cx("replica-create")}>
          ...
          <section className={cx("replica-calendar")}>
            <header>
              <h2>Upcoming Calendar</h2>
              <button type="button" onClick={() => moveToSection("Calendar")}>
                View All
              </button>
            </header>
            {[
              ["Independence Day Offer", "15 Aug 2026 · All Day"],
              ["Back to School Campaign", "20 Aug 2026 · 10:00 AM"],
              ["Weekend Special Offer", "22 Aug 2026 · All Day"],
            ].map(([title, date], index) => (
              <button
                type="button"
                key={title}
                onClick={() => moveToSection("Calendar")}
              >
          ...
          <section className={cx("replica-trending")}>
            <header>
            ...
            {trendingTags.length ? (
              trendingTags.map(([tag, count]) => (
                <button key={tag} type="button">
                  <strong>#{String(tag).replace(/^#/, "")}</strong>
                  <small>
              ...
              <p className={cx("replica-muted")}>
                Hashtag counts will appear from published posts.
              </p>
            )}
          </section>
        </aside>
        <button
          className={cx("replica-fab")}
          type="button"
          onClick={openComposer}
          aria-label="Create Spotlight post"
        >
          ✎
        </button>
        {selectedStory !== null && (
          <div
            className={cx("status-backdrop")}
            role="presentation"
            onMouseDown={() => setSelectedStory(null)}
          >
            <section
              className={cx("status-viewer")}
              role="dialog"
              aria-modal="true"
              aria-label={`${stories[selectedStory].name} status`}
              onMouseDown={(event) => event.stopPropagation()}
            >
              <div
                className={cx("status-progress")}
              ...
              <header>
                <span className={cx("status-avatar")}>
                  {stories[selectedStory].name.charAt(0)}
                </span>
                <div>
                  <strong>
                    {stories[selectedStory].mine
                      ? "Your Story"
                      : stories[selectedStory].name}
                  </strong>
                  <small>
                    {new window.Intl.DateTimeFormat(undefined, {
                      hour: "numeric",
                      minute: "2-digit",
                    }).format(new Date(stories[selectedStory].post.createdAt))}
                  </small>
                </div>
                {stories[selectedStory].mine && (
                  <>
                    <button
                      type="button"
                      onClick={() => {
                        setStoryEditor(stories[selectedStory].post);
                        setSelectedStory(null);
                      }}
                    >
                      Edit
                    ...
                    <button
                      ...
                      disabled={storyBusy}
                      onClick={() =>
                        removeSelectedStory(stories[selectedStory].post)
                      }
              ...
              <div
                ...
                style={{
                  background:
                    stories[selectedStory].post.storyStyle?.background,
                  color: stories[selectedStory].post.storyStyle?.textColor,
                  textAlign: stories[selectedStory].post.storyStyle?.align,
                }}
              >
                {stories[selectedStory].image ? (
                  stories[selectedStory].mediaType === "video" ? (
                    <video
                ...
                <div
                  className={cx(
                    `story-font-${stories[selectedStory].post.storyStyle?.font || "modern"}`,
                  )}
                >
                  <strong>{stories[selectedStory].post.text}</strong>
                  {stories[selectedStory].post.storyStyle?.musicTitle && (
                    <small>
                      ♫ {stories[selectedStory].post.storyStyle.musicTitle}
                      {stories[selectedStory].post.storyStyle.musicArtist
                        ? ` · ${stories[selectedStory].post.storyStyle.musicArtist}`
                        : ""}
                    </small>
                  )}
              ...
              <button
                className={cx("status-prev")}
                type="button"
                disabled={selectedStory === 0}
                onClick={() => showStory(selectedStory - 1)}
                aria-label="Previous status"
              >
                <ChevronLeft />
              </button>
              <button
                className={cx("status-next")}
                type="button"
              ...
              <footer>
                <input placeholder="Reply to this status…" />
                <button type="button" aria-label="Like status">
                  <Heart />
                </button>
                <button type="button" aria-label="Share status">
                  <Send />
                </button>
              </footer>
            </section>
        ...
        {storyEditor !== null && (
          <StoryEditor
            ...
            onPublish={submitStory}
          />
        )}
        {profilePostId &&
          (() => {
            const profilePost = posts.find(
              (item) => item._id === profilePostId,
            );
            return profilePost ? (
              <BusinessProfileModal
                post={profilePost}
                posts={posts}
                onClose={() => setProfilePostId(null)}
                onFollow={handleFollow}
              />
            ) : null;
          })()}
        {selectedBusinessId &&
          (() => {
            const selectedBusiness = businesses.find(
              (item) => String(item.businessId) === String(selectedBusinessId),
            );
...
const matchesFeedFilter = (post = {}, filter = "All") => {
  ...
  if (filter === "Offers" && (post.offerStartsAt || post.offerEndsAt))
    return true;
  if (
    filter === "Videos" &&
    post.mediaItems?.some((item) => item.mediaType === "video")
  )
    return true;
  return (
    String(post.postType || "update").toLowerCase() ===
    FILTER_POST_TYPES[filter]
  );
};
...
function FeedMediaItem({ item, post, className, stopPropagation = false }) {
  ...
  if (item.mediaType === "file")
    return (
      <a
        className={cx("post-file-attachment")}
        href={item.mediaUrl}
        target="_blank"
        rel="noreferrer"
        download
      ...
      >
        ...
        <strong>{item.fileName || "Download attachment"}</strong>
  ...
  return (
    <img
      className={className}
      src={item.mediaUrl}
      alt={item.fileName || post.title || "Spotlight"}
      loading="lazy"
    />
  );
}
  ...
  return (
    ...
    >
      <div className={cx("post-body")}>
        <div className={cx("post-header")}>
          ...
          <div className={cx("post-identity")}>
            <div className={cx("post-name-row")}>
              ...
              <span
                className={cx(
                  "post-type-badge",
                  `post-type-${getPostTypeClass(post)}`,
                )}
              >
                {getPostKind(post)}
              </span>
          ...
          {post.businessId && (
            <button
              className={cx(
                "follow-button",
                isFollowing && "follow-button-active",
              )}
              type="button"
              onClick={(event) => {
                event.stopPropagation();
                onFollow(post.businessId);
              }}
            >
        ...
        {!!post.callToActions?.length && (
          <div
            className={cx("post-cta-row")}
            onClick={(event) => event.stopPropagation()}
          >
            {post.callToActions.map((item) => (
              <a
                href={getActionHref(item)}
                target={item.action === "call" ? undefined : "_blank"}
                rel="noreferrer"
                key={item.action}
              >
                {item.label}
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="spotlight-detail-title"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button
          className={cx("detail-close")}
          type="button"
          onClick={onClose}
          aria-label="Close Spotlight details"
        >
          <CloseIcon />
        </button>
        <div className={cx("detail-media")}>
          {post.mediaItems?.length ? (
            <div
              className={cx(
                "detail-media-grid",
                `detail-media-${Math.min(post.mediaItems.length, 4)}`,
              )}
            >
              {post.mediaItems.map((item) => (
                <FeedMediaItem key={item.mediaKey} item={item} post={post} />
              ))}
            </div>
          ) : (
            <div className={cx("detail-media-empty")}>
              <span>{(post.businessName || "M").charAt(0)}</span>
              <p>Local business update</p>
            </div>
          )}
        </div>
        <div className={cx("detail-content")}>
          <header className={cx("detail-business")}>
            ...
            <div>
              <strong>{post.businessName || "MassClick Business"}</strong>
              <span>
                {[post.businessCategory, post.businessLocation]
                  .filter(Boolean)
                  .join(" · ") || "Local business"}
          ...
          <div className={cx("detail-scroll")}>
            {offerText && (
              <div className={cx("detail-offer")}>
                <LocalOfferIcon fontSize="small" />
                <div>
                  <span>Offer validity</span>
                  <strong>{offerText.replace("Offer: ", "")}</strong>
                </div>
              </div>
            )}
            <div className={cx("detail-copy")}>
              <h2 id="spotlight-detail-title">
            ...
            {!!post.callToActions?.length && (
              <div className={cx("detail-cta-row")}>
                {post.callToActions.map((item) => (
                  <a
                    href={getActionHref(item)}
                    target={item.action === "call" ? undefined : "_blank"}
            ...
            <div className={cx("detail-comments")}>
              <div className={cx("detail-section-title")}>
                <h3>Comments</h3>
                <span>{post.commentsCount || 0}</span>
              </div>
              {(post.comments || []).length ? (
                (post.comments || []).map((comment) => (
                  <div className={cx("detail-comment")} key={comment._id}>
                    <span>{(comment.userName || "U").charAt(0)}</span>
                    <div>
                ...
                <p className={cx("detail-no-comments")}>
          ...
          <div className={cx("detail-actions")}>
            <button
              type="button"
              className={cx(post.likedByMe && "detail-liked")}
    ...
    >
      ...
      >
        <header className={cx("data-modal-header")}>
          <div>
            <span>Live Spotlight data</span>
            <h2 id="live-data-title">{view.title}</h2>
            <p>{view.description}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close live data">
            <CloseIcon />
          </button>
        </header>
        <div className={cx("data-modal-summary")}>
          <strong>{view.value}</strong>
          <span>{view.label || view.title}</span>
        </div>
          ...
          {view.kind === "businesses" ? (
            businesses.map((business) => (
              <button
                ...
                key={business.key}
                onClick={() => {
                  onFilter("business", business.name);
                  onClose();
                }}
              >
                <span className={cx("data-avatar")}>
            ...
            visiblePosts.map((post) => (
              ...
              >
                ...
                <span>
                  <strong>
                    {post.title || post.businessName || "Spotlight update"}
                  </strong>
                  <small>
                    {post.businessName} · {formatDate(post.createdAt)}
                  </small>
                </span>
                <b>
                  {view.postValue ? view.postValue(post) : getPostKind(post)}
                </b>
              </button>
            ))
          ) : (
            <div className={cx("data-empty")}>
              <strong>No matching records yet</strong>
              <p>
                This value is calculated from the live Spotlight posts returned
                by the database.
              </p>
            </div>
          )}
...
export default function MassclickFeedPage() {
  ...
  const filteredPosts = posts.filter(
    (post) =>
      post.postType !== "story" &&
      matchesFeedFilter(post, activeFilter) &&
      (!businessFilter || post.businessName === businessFilter),
  );
  const selectedPost =
    posts.find((post) => post._id === selectedPostId) || null;

  const openDataView = (title, value, description, options = {}) =>
    setDataView({ title, value, description, ...options });
  const applyDataFilter = (type, value) => {
    if (type === "category") {
      setBusinessFilter("");
      const matchingType = Object.keys(FILTER_POST_TYPES).find(
        (label) => FILTER_POST_TYPES[label] === String(value).toLowerCase(),
      );
  ...
  useEffect(() => {
    ...
    dispatch(getMassclickFeedBusinesses({ page: 1, limit: 10 }))
      .then((result) => {
  ...
  const loadBusinessPage = async (page, search = businessQuery) => {
    ...
    try {
      const result = await dispatch(
        getMassclickFeedBusinesses({ page, limit: 10, search }),
      );
      ...
      if (requestId === businessRequestRef.current) setBusinessLoading(false);
    }
  };

  const searchBusinesses = (search) => {
    setBusinessQuery(search);
    loadBusinessPage(1, search);
  };

  const toggleFollow = async (businessId, isFollowing) => {
    const result = await dispatch(
      setMassclickFeedFollow(businessId, !isFollowing),
    );
    setSuggestedBusinesses((items) =>
      items.map((business) =>
  ...
  const moveToSection = (name) => {
    ...
    const route = {
      "Spotlight Feed": "/user_feed",
      "Planner Overview": "/user_spotlight/calendar",
      Planner: "/user_spotlight/calendar",
      "My Posts": "/user_spotlight/posts",
      Media: "/user_spotlight/media",
      "Media Library": "/user_spotlight/media",
      Settings: "/user_spotlight/settings",
    }[name];
    if (route) {
      navigate(route);
      return;
  ...
  const runTool = (title) => {
    const route = {
      "Post Scheduler": "/user_spotlight/calendar",
      "Media Library": "/user_spotlight/media",
    }[title];
  ...
  return (
    <SpotlightReplica
      ...
      handleFollow={(businessId, isFollowing) =>
        toggleFollow(businessId, isFollowing)
      }
      openPost={(post) => {
        dispatch(recordMassclickFeedView(post._id));
        setSelectedPostId(post._id);
      }}
      createStory={(payload) => dispatch(createMassclickFeedPost(payload))}
      editStory={(postId, payload) =>
        dispatch(updateMassclickFeedStory(postId, payload))
      }
    ...
    return (
      <>
        ...
        <main className={cx("feed-page")}>
          <aside
            className={cx("command-rail")}
            aria-label="Spotlight command navigation"
          >
            <button className={cx("command-rail-active")} type="button">
              <i>✦</i>
              <span>Spotlight</span>
            </button>
            {[
              ...
              <button
                ...
                key={label}
                onClick={
                  label === "Create" ? openComposer : () => moveToSection(label)
                }
              >
                <i>{icon}</i>
                <span>{label}</span>
          ...
          <div className={cx("feed-topbar")}>
            <div className={cx("spotlight-heading")}>
              ...
              <div>
                ...
                <p>
                  Promote your business, offers, updates and connect with local
                  customers.
                </p>
              </div>
            </div>
            <nav
              className={cx("command-tabs")}
            ...
            <button className={cx("command-date")} type="button">
              {new Intl.DateTimeFormat(undefined, {
                weekday: "short",
                day: "2-digit",
                month: "short",
          ...
          <div className={cx("feed-shell")}>
            <aside className={cx("left-rail")} aria-label="Feed discovery">
              <section
                className={cx("rail-panel rail-panel-dark side-menu-panel")}
              >
                <div className={cx("side-brand")}>
                  <strong>
                    MassCli<span>ck</span>
                  </strong>
                ...
                <nav className={cx("spot-nav")}>
                  {spotlightNavigation.map(([icon, label]) => (
                    <Link
                      key={label}
                      to={spotlightRoutes[label] || "/"}
                      className={cx(
                        "spot-nav-item",
                        label === "Spotlight" && "spot-nav-active",
                      )}
                    >
                      <span>{icon}</span>
                      {label}
                ...
                <div className={cx("visibility-card")}>
                  ...
                  <p>Reach more customers and grow your business faster.</p>
                  <button type="button">Upgrade Now</button>
                </div>
              </section>
                  ...
                  {topCategories.length ? (
                    topCategories.map(([category, count]) => (
                      <button
                        ...
                        key={category}
                        onClick={() => applyDataFilter("category", category)}
                      >
                        <span>{category}</span>
                        <strong className={cx("category-count")}>
                          {count}
                        </strong>
                      </button>
                    ))
            ...
            <div className={cx("feed-column")}>
              <section className={cx("feed-hero")}>
                <div className={cx("hero-copy")}>
                  ...
                  <h1 className={cx("feed-title")}>
                    Make local discovery feel alive.
                  </h1>
                  <p className={cx("feed-subtitle")}>
                    Real-time offers, announcements, and updates from nearby
                    businesses. Discover what&apos;s happening in your community
                    right now.
                  </p>
                  <div className={cx("hero-tags")}>
                    <span className={cx("hero-tag")}>
                ...
                <div className={cx("hero-action-card")}>
                  <span className={cx("hero-action-label")}>
                    Business owner?
                  </span>
              ...
              <section className={cx("feed-toolbar")}>
                <div>
                  ...
                  <h2 className={cx("toolbar-title")}>
                    Latest verified updates
                  </h2>
                </div>
                <div className={cx("toolbar-filters")}>
                  <span className={cx("toolbar-count")}>
                    {posts.length} posts
                  </span>
                  {offerCount > 0 && (
                    <span
                ...
                >
                  {[
                    ...
                    <button
                      ...
                      className={cx(
                        "category-tab",
                        activeFilter === filter && "category-tab-active",
                      )}
                      onClick={() => {
                        setActiveFilter(filter);
                        setBusinessFilter("");
              ...
              <section
                className={cx("reference-kpis")}
                aria-label="Spotlight performance overview"
              >
                {[
                  ...
                  <button
                    className={cx("reference-kpi-card")}
                    type="button"
                    key={label}
                    onClick={() =>
                      openDataView(
                        label,
                        value,
                        `Calculated from ${posts.length} live Spotlight post${posts.length === 1 ? "" : "s"}.`,
                        {
                          postValue:
                            label === "Views"
                              ? (post) =>
                                  `${post.viewsCount || post.viewCount || post.views || 0} views`
                              : label === "Engagements"
                                ? (post) =>
                                    `${(post.likesCount || 0) + (post.commentsCount || 0) + (post.sharesCount || 0)} engagements`
                                : label === "Leads Generated"
                                  ? (post) =>
                                      `${post.leadsCount || post.leadCount || 0} leads`
                                  : undefined,
                        },
                      )
                    }
                  >
                    <i className={cx(`reference-kpi-${icon}`)}>
                      {icon === "eye"
                        ? "◉"
                        : icon === "heart"
                          ? "♡"
                ...
                {topPost && (
                  <button
                    type="button"
                    className={cx("reference-top-performer")}
                    onClick={() => setSelectedPostId(topPost._id)}
                  >
                    <span className={cx("performer-art")}>MC</span>
                    <div>
                      <small>Top Performer</small>
                      <strong>{topPost.title || topPost.businessName}</strong>
                      <b>
                        {(topPost.likesCount || topPost.likes?.length || 0) +
              ...
              <section
                className={cx("reference-posts-panel")}
                ref={feedSectionRef}
              >
                <div className={cx("section-line")}>
                  <h2>
                    {businessFilter
                      ? `${businessFilter} posts`
                      : "Spotlight Posts"}
                  </h2>
                  <div className={cx("carousel-heading-actions")}>
                    {businessFilter && (
                      <button
                        type="button"
                    ...
                    <button
                      ...
                      aria-label="Show previous Spotlight post"
                    >
                      ‹
                    </button>
                    <button
                      type="button"
                      onClick={() => scrollPosts(1)}
                      aria-label="Show next Spotlight post"
                    >
                      ›
                    </button>
                  </div>
                  ...
                  {filteredPosts.length ? (
                    filteredPosts.map((post) => (
                      <FeedPost
                        ...
                        onFollow={() =>
                          toggleFollow(post.businessId, post.isFollowing)
                        }
                      />
                    ))
                  ) : (
              ...
              <section className={cx("tools-panel")} ref={toolsSectionRef}>
                ...
                <div className={cx("tools-grid")}>
                  {spotlightTools.map(([icon, title, text]) => (
                    <button
                      type="button"
                      key={title}
              ...
              <section className={cx("ai-assistant")} ref={assistantSectionRef}>
                <div className={cx("assistant-intro")}>
                  <i>✦</i>
                  <span>
                    <strong>AI Spotlight Assistant</strong>
                    <small>Get smart suggestions to grow your business</small>
                  </span>
                  <button type="button" onClick={openComposer}>
                ...
                {topCategories.map(([category, count]) => (
                  <div className={cx("assistant-tip")} key={category}>
                    <i>⌘</i>
                    <span>
              ...
              <section className={cx("featured-panel")}>
                <div className={cx("section-line")}>
                  ...
                  {featuredBusinesses.length > 7 && (
                    <button
                      type="button"
                      onClick={() => setShowAllBusinesses((value) => !value)}
                    >
                      {showAllBusinesses ? "Show Less" : "View All"}
                    </button>
                  )}
                </div>
                <div className={cx("featured-row")}>
                  {featuredBusinesses.length ? (
                    (showAllBusinesses
                      ? featuredBusinesses
                      : featuredBusinesses.slice(0, 7)
                    ).map((business) => (
                      <button
                        type="button"
                        className={cx("featured-business")}
                        key={business.key}
                        onClick={() =>
                          applyDataFilter("business", business.name)
                        }
                      >
                        {business.image ? (
                    ...
                    <p className={cx("featured-empty")}>
                      Featured businesses will appear when businesses publish
                      Spotlight posts.
                    </p>
                  )}
                </div>
              ...
              <section className={cx("bottom-metrics")}>
                {[
                  [
                    featuredBusinesses.length.toLocaleString(),
                    "Active Businesses",
                  ],
                  [posts.length.toLocaleString(), "Spotlight Posts"],
                  [totalEngagements.toLocaleString(), "Total Engagements"],
                  [totalViews.toLocaleString(), "Total Views"],
                  [totalLeads.toLocaleString(), "Leads Generated"],
                  [
                  ...
                  <button
                    ...
                    onClick={() =>
                      openDataView(
                        label,
                        value,
                        "This total uses only records returned by the live Spotlight API.",
                        {
                          kind:
            ...
            <aside className={cx("right-rail")} aria-label="Posting guidance">
              ...
              <section
                className={cx("analytics-panel")}
                ref={analyticsSectionRef}
              >
                <div className={cx("analytics-title")}>
                  <h2>Spotlight Analytics</h2>
                  <span>Live data</span>
                </div>
                <div className={cx("analytics-grid")}>
                  {[
                    ...
                    [formatCompactNumber(totalLeads), "Leads Generated"],
                  ].map(([value, label]) => (
                    <button
                      type="button"
                      key={label}
                      onClick={() =>
                        openDataView(
                          label,
                          value,
                          "Live total calculated from the currently loaded database records.",
                        )
                      }
                    >
                      <i>◉</i>
              ...
              <section className={cx("audience-panel")}>
                ...
                <h3>Categories represented in the live feed</h3>
                <div className={cx("audience-locations")}>
                  <ul>
                    {topCategories.map(([name, value]) => (
                      <li key={name}>
                        <button
                          type="button"
                          onClick={() => applyDataFilter("category", name)}
                        >
                          <i />
                          {name}
                          <b>{value}</b>
              ...
              <section className={cx("rail-calendar")} ref={calendarSectionRef}>
                <div className={cx("section-line")}>
                  ...
                  <button
                    type="button"
                    onClick={() => moveToSection("Planner")}
                  >
                    View Calendar
                  </button>
                </div>
                <div className={cx("rail-calendar-days")}>
                  {posts.slice(0, 7).map((post) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{post.title || getPostKind(post)}</span>
                      </button>
                    );
                  })}
                </div>
              </section>
              <section className={cx("recent-activity")}>
                <div className={cx("section-line")}>
                  <h2>Recent Posts</h2>
                  <button type="button" onClick={() => moveToSection("Feed")}>
                    View All
                  </button>
                </div>
                <div>
                  {posts.slice(0, 3).map((post) => (
                    ...
                    >
                      <i>●</i>
                      <b>
                        {post.title || post.businessName || "Spotlight post"}
                      </b>
                ...
                <div className={cx("playbook-list")}>
                  {playbookItems.map(({ icon: Icon, title, text }) => (
                    <div className={cx("playbook-item")} key={title}>
                      ...
                      <div>
                        <strong className={cx("playbook-title")}>
                          {title}
                        </strong>
              ...
              <section className={cx("rail-panel rail-cta")}>
                ...
                <h2 className={cx("rail-cta-title")}>
                  Turn every feed post into a reason to visit, call, book, or
                  buy.
                </h2>
                <button
          ...
          <section className={cx("command-bottom-strip")}>
            {[
              ...
              ["↗", "Shares", totalShares],
              ["♙", "Businesses", featuredBusinesses.length],
            ].map(([icon, label, value]) => (
              <div key={label}>
                <i>{icon}</i>
                <span>
