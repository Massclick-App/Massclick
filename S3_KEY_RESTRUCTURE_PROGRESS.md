...
**2026-08-14: category rehearsal actually run for real** (908/908 clean through copy/verify-s3/rewrite/verify,
left on new keys on dev — see "2026-08-14 — category rehearsal run for real" below). **2026-08-14: all of
2.3's previously-open gaps closed for real** — heartbeat proven live (30s tick observed), pause/cancel
proven live (`isStopRequested` flips the instant status changes), SIGKILL-mid-copy proven (killed at 1/50,
resumed cleanly to 50/50), and a tunnel-drop bug was found and fixed (see below). **Two real bugs found
and fixed by this session's resilience testing** — neither was a data-corruption risk, both are now closed:
1. `failJob` calls in `copy`/`rewrite`'s error paths were unguarded, so if the SAME connection used for
   `--state-uri` was also the one that just died, the job-doc-write-on-failure itself threw, masking the
   documented clean "tunnel down, reconnect and re-run" message behind a raw uncaught stack trace. Fixed:
   all 6 `tracker.failJob/finishJob/releaseSlot` call sites in `s3KeyMigration.js` now `.catch(() => {})`,
   matching the pattern already used for `updateProgress`.
2. `verify`'s field-read used `{ [row.locator]: 1 }` as the Mongo projection for EVERY locator, including
   array-index ones like `ratingPhotos.41`. Mongo's array-index projection re-indexes the result to a
   single-element array starting at 0 — so `doc.ratingPhotos[41]` on the projected doc came back `undefined`
   even though the live, unprojected document held the correct value at the real index all along. This
   would have given false FAILs on every array-index-locator scope during the real R.5/R.9 verify. Fixed:
   project only the base field (`ratingPhotos`, not `ratingPhotos.41`), matching how `scan`/`plan` already
   build their projections from the registry.
**Still missing entirely:** `sweep`/`undelete`/`restore-from-local` — low priority, only needed at S.3,
deliberately deferred. **The one item still open before Track A is fully closed: the card has never been
viewed in a browser** — needs the user, per the project's standing no-browser-tools rule.
...
| 2.3 | Monitoring card + 5 admin endpoints (no `/start`) | 🟡 **heartbeat + pause/cancel now proven live**; card-in-browser still untested | 30s heartbeat tick observed directly; `isStopRequested` proven to flip on a status change; job-doc create/update/complete proven for copy/rewrite; found+fixed a real `counts.done` sync bug live |
| 3 | **Rehearsal: `advertisements/`** + SIGKILL ×2 + tunnel drop | 🟡 **full cycle done; resilience tests run on `businessReviews` instead** (see below) | reverse proven · resume proven · SIGKILL-mid-copy proven · tunnel-drop bug found+fixed |
| 4 | **Rehearsal: `category/`** full cycle | ✅ **DONE 2026-08-14, real run** | 908/908 clean through copy/verify-s3/rewrite/verify, dev left on new keys, 0 newly broken images |
