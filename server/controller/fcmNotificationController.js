...
async function dispatchToken(token, title, body, data = {}) {
  if (typeof token === 'string' && token.startsWith('{')) {
    ...
    const payload = JSON.stringify({ notification: { title, body, icon: '/apple-touch-icon.png', data } });
...
export const sendBulkNotificationAction = async (req, res) => {
  try {
    ...
    const webPayload = JSON.stringify({ notification: { title, body, icon: '/apple-touch-icon.png', data: data || {} } });
