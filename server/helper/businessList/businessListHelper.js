...
export const updateBusinessList = async (id, data) => {
  ...
  if (Array.isArray(data.payment) && data.payment.length > 0) {
    if (!business.amountPaid) {
      ...
      // Paid businesses automatically receive verified + trust status here,
      // in the mark-paid path itself, so certificates get generated by
      // ensureCertificatesForActivation even if the invoice email fails.
      if (!business.verification?.isVerified) {
        business.verification = {
          ...(business.verification?.toObject?.() || business.verification || {}),
          isVerified: true,
          verifiedAt: new Date(),
      ...
      business.badges = {
        ...(business.badges?.toObject?.() || business.badges || {}),
        isTrust: true,
      };
